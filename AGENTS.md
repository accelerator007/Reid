...
- The Production QR service accepts group messages only from the exact database-registered `Reid_Owner` JID and only when the authenticated sender is an enabled active Owner. That immutable JID is pinned by migration `202609120004_owner_group_always_active.sql`; mutable WhatsApp display-subject normalization is no longer part of the live decision. The group is always-on for Ali and Sheikha without requiring a mention, while every other sender and group remains silent. An unregistered group can still enter only through an explicit name/mention bootstrap and exact configured subject.
