...
try{
  ...
  for(const [route,heading] of [['today','أهلًا'],['finance','وضوح'],['operations','التفاصيل'],['assistant','فكرتك'],['connections','كل أدواتك'],['inbox','كل محادثة']]){
    await page.goto(`http://127.0.0.1:5175/${route}`);await page.getByRole('heading',{name:new RegExp(heading)}).first().waitFor({timeout:20000});await pause(1500);await page.screenshot({path:`${folder}/${route}.png`,fullPage:true});console.log(`${route}: rendered`);
  }
