...
try{
  ...
  await page.goto(`${base}/finance`);await page.getByRole('button',{name:'مستند جديد'}).click();await page.locator('[name=title]').fill('QA invoice - delete after test');await page.locator('[name=counterparty]').fill('Synthetic customer');await page.locator('[name=amount]').fill('12.345');await page.getByRole('button',{name:'حفظ كمسودة'}).click();await page.getByText('QA invoice - delete after test',{exact:true}).waitFor();console.log('Finance: UI create persisted');
  await page.getByRole('button',{name:'المصروفات'}).click();await page.getByRole('button',{name:'مستند جديد'}).click();await page.locator('[name=title]').fill('QA expense - delete after test');await page.locator('[name=counterparty]').fill('Synthetic supplier');await page.locator('[name=amount]').fill('4.321');await page.getByRole('button',{name:'حفظ كمسودة'}).click();await page.getByText('QA expense - delete after test',{exact:true}).waitFor();let expenseRow=page.locator('.os-record').filter({hasText:'QA expense - delete after test'});await expenseRow.locator('select').selectOption('issued');await page.waitForTimeout(500);expenseRow=page.locator('.os-record').filter({hasText:'QA expense - delete after test'});await expenseRow.locator('select').selectOption('paid');await page.waitForTimeout(500);expenseRow=page.locator('.os-record').filter({hasText:'QA expense - delete after test'});if(await expenseRow.locator('select').inputValue()!=='paid')throw Error('expense_paid_transition_failed');console.log('Finance: expense issued and paid through UI');
